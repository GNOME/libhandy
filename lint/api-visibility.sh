...
for header in $(find src -name '*.h' -not -regex '.*private.h' -not -regex '.*/handy.h' | sed 's/src\///');
do
  if ! grep -q "$header" src/handy.h;
  then
    echo "The public header" $header "should be included in handy.h."
    exit 1
  fi
done
