...
/*
 * Forked from the GTK+ 3.24.2 GtkStack widget and modified for libhandy by
 * Adrien Plazas on behalf of Purism SPC 2019.
 */
...
hdy_squeezer_class_init (HdySqueezerClass *klass)
{
  ...
  child_props[CHILD_PROP_ENABLED] =
    g_param_spec_boolean ("enabled",
                          _("Enabled"),
                          _("Whether the child can be picked or should be ignored when looking for the child fitting the available size the best"),
                          TRUE,
                          G_PARAM_READWRITE);
                                ...
                                GtkWidget   *child)
{
  ...
  child_info = find_child_info_for_widget (self, child);
  if (child_info == NULL) {
    g_warning ("Given child of type '%s' not found in HdySqueezer",
 ...
 *
 * Make @self enabled or diabled @child. If a child is disabled, it will be
 * ignored by the size request and allocation algorithms, as if it wasn't a
 * child of @self or as if it was hidden. This allows to programmatically and
 * prematurely hide a child of @self even if it fits in the available space.
 *
