 ...
 * .isolated style class, and also will add the .isolated-previous-sibling style
 * class to its previous sibling, and remove it when not isolated.
 *
 * Since: 0.0.10
 */
...
update_style_classes (HdyPreferencesRow *self)
{
  ...
  set_style_class (GTK_WIDGET (self), "isolated", priv->isolated);

  if (previous_sibling)
    set_style_class (previous_sibling, "isolated-previous-sibling", priv->isolated);
}
