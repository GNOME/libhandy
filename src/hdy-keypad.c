                         ...
                         GParamSpec   *pspec)
{
  ...
  case PROP_ONLY_DIGITS:
    if (g_value_get_boolean (value) != priv->only_digits) {
      ...
      if (priv->only_digits)
        hdy_keypad_show_symbols (self, FALSE);
...
static GtkWidget *
image_for_custom_icon (const gchar *icon_name)
{
  g_autofree gchar *resource_uri;
  g_autoptr (GFile) file;
  g_autoptr (GIcon) icon;
  GtkWidget *image;

  resource_uri = g_strconcat ("resource:///sm/puri/handy/images/hdy-keypad-",
                              icon_name,
                              "-symbolic.svg",
                              NULL);
...
hdy_keypad_constructed (GObject *object)
{
  ...
  gtk_container_add (GTK_CONTAINER (priv->btn_asterisk), image_for_custom_icon ("asterisk"));
  gtk_container_add (GTK_CONTAINER (priv->btn_hash), image_for_custom_icon ("hash"));
...
hdy_keypad_class_init (HdyKeypadClass *klass)
{
  ...
  gtk_widget_class_bind_template_child_private (widget_class, HdyKeypad, btn_asterisk);
  gtk_widget_class_bind_template_child_private (widget_class, HdyKeypad, btn_hash);
...
hdy_keypad_set_entry (HdyKeypad *self, GtkEntry *entry)
{
  ...
  g_object_set (priv->entry, "im-module", "gtk-im-context-simple", NULL);
