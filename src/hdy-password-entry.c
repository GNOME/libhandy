...
static void
box_size_allocated_cb (GtkWidget    *widget,
                       GdkRectangle *allocation,
                       gpointer      user_data)
{
					               ...
					               HdyPasswordEntry *entry)
{
  ...
  if (priv->show_caps_icon && gdk_keymap_get_caps_lock_state (keymap))
    gtk_widget_show (priv->caps_icon_event_box);
  else
    gtk_widget_hide (priv->caps_icon_event_box);
}
                                  ...
                                  GParamSpec *pspec)
{
  ...
  case PROP_SHOW_CAPS_ICON:
    g_value_set_boolean (value, hdy_password_entry_get_show_caps_icon (self));
    break;
                                 ...
                                 GParamSpec   *pspec)
{
  ...
  case PROP_SHOW_CAPS_ICON:
    hdy_password_entry_set_show_caps_icon (self, g_value_get_boolean (value));
    break;
...
hdy_password_entry_class_init (HdyPasswordEntryClass *klass)
{
  ...
  /**
   * HdyPasswordEntry::show-caps-icon:
   *
   * Whether to show the caps icon.
   *
   * Changing this property sends a notify signal.
   *
   * Since: 1.0
   */
  props[PROP_SHOW_CAPS_ICON] = g_param_spec_boolean
                                 ("show-caps-icon",
                                  _("Show caps icon"),
                                  _("Whether to show the caps icon"),
                                  TRUE,
                                  G_PARAM_READWRITE);
...
hdy_password_entry_init (HdyPasswordEntry *entry)
{
  ...
  priv->caps_icon = gtk_image_new_from_icon_name ("caps-lock-symbolic", GTK_ICON_SIZE_MENU);
  priv->caps_icon_event_box = gtk_event_box_new ();
  priv->box = gtk_box_new (GTK_ORIENTATION_HORIZONTAL, 6);
  keymap = gdk_keymap_get_for_display (gtk_widget_get_display (priv->password_entry));
  ...
  gtk_widget_set_halign (priv->box, GTK_ALIGN_END);

  gtk_widget_set_tooltip_text (priv->peek_icon_event_box, _("Show text"));
  gtk_widget_set_tooltip_text (priv->caps_icon_event_box, _("Caps Lock is on"));
  ...
  gtk_widget_set_valign (priv->caps_icon, GTK_ALIGN_CENTER);

  g_signal_connect (G_OBJECT (priv->box), "size-allocate",
                    G_CALLBACK (box_size_allocated_cb), entry);
  g_signal_connect (G_OBJECT (priv->password_entry), "size-allocate",
                    G_CALLBACK (password_entry_size_allocated_cb), priv->box);
  ...
  gtk_overlay_add_overlay (GTK_OVERLAY (entry), priv->box);
  gtk_container_add (GTK_CONTAINER (entry), priv->password_entry);
  gtk_box_pack_end (GTK_BOX (priv->box), priv->peek_icon_event_box, FALSE, FALSE, 0);
  gtk_box_pack_end (GTK_BOX (priv->box), priv->caps_icon_event_box, FALSE, FALSE, 0);
  gtk_container_add (GTK_CONTAINER (priv->peek_icon_event_box), priv->peek_icon);
  gtk_container_add (GTK_CONTAINER (priv->caps_icon_event_box), priv->caps_icon);
  ...
  gtk_widget_show (priv->caps_icon);

  g_signal_connect (keymap, "state-changed", G_CALLBACK (keymap_state_changed_cb), entry);
}
                                       ...
                                       gboolean show_peek_icon)
{
  ...
  if (show_peek_icon)
    ...
    gtk_widget_hide (priv->peek_icon_event_box);

  priv->show_peek_icon = show_peek_icon;
}

/**
 * hdy_password_entry_get_show_caps_icon:
 * @entry: a HdyPasswordEntry
 *
 * Returns whether the entry is showing an icon to indicate
 * the caps lock status.
 *
 * Since: 1.0
 */
gboolean
hdy_password_entry_get_show_caps_icon (HdyPasswordEntry *entry)
{
  HdyPasswordEntryPrivate *priv = hdy_password_entry_get_instance_private (entry);

  return priv->show_caps_icon;
}

/**
 * hdy_password_entry_set_show_caps_icon:
 * @entry: a HdyPasswordEntry
 * @show_caps_icon: whether to show the peek icon
 *
 * Sets whether the entry should have an icon to indicate
 * the caps lock status.
 *
 * Since: 1.0
 */
void
hdy_password_entry_set_show_caps_icon (HdyPasswordEntry *entry,
                                       gboolean show_caps_icon)
{
  HdyPasswordEntryPrivate *priv = hdy_password_entry_get_instance_private (entry);
  GdkKeymap *keymap;

  if (show_caps_icon == priv->show_caps_icon)
    return;

  keymap = gdk_keymap_get_for_display (gtk_widget_get_display (priv->password_entry));

  if (show_caps_icon && gdk_keymap_get_caps_lock_state (keymap))
    {
      gtk_widget_show (priv->caps_icon_event_box);
      g_signal_connect (keymap, "state-changed", G_CALLBACK (keymap_state_changed_cb), entry);
    }
  ...
  priv->show_caps_icon = show_caps_icon;
}
