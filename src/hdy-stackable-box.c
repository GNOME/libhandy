 ...
 * @stability: Private
 *
 * The #HdyStackableBox widget can display its children like a #GtkBox does or
 * like a #HdyStackableBox does, adapting to size changes by switching between
 * the two modes.
 *
  ...
  if (!self->child_transition.is_gesture_active &&
    ...
    if (self->child_transition.is_cancelled) {
      if (self->last_visible_child != NULL) {
        if (hdy_stackable_box_get_fold (self) == HDY_FOLD_FOLDED) {
 ...
 */
HdyFold
hdy_stackable_box_get_fold (HdyStackableBox *self)
{
...
static void
hdy_stackable_box_set_fold (HdyStackableBox *self,
                            HdyFold          fold)
{
  GtkStyleContext *context;

  g_return_if_fail (HDY_IS_STACKABLE_BOX (self));
/* FIXME: fix https://source.puri.sm/Librem5/libhandy/issues/198 and uncomment this
  if (self->fold == fold)
    return;
*/
  self->fold = fold;

  hdy_stackable_box_start_mode_transition (self, fold ? 0.0 : 1.0);
                                   ...
                                   gboolean         homogeneous)
{
  ...
  if (self->homogeneous[fold][orientation] == homogeneous)
    return;

  self->homogeneous[fold][orientation] = homogeneous;
                                   ...
                                   GtkOrientation   orientation)
{
  ...
  return self->homogeneous[fold][orientation];
}
                                 ...
                                 GtkAllocation   *allocation)
{
  ...
  hdy_stackable_box_set_fold (self, folded ? HDY_FOLD_FOLDED : HDY_FOLD_UNFOLDED);
                        ...
                        cairo_t         *cr)
{
  ...
  if (self->fold == HDY_FOLD_UNFOLDED)
                       ...
                       GtkWidget       *widget)
{
  ...
  if (self->fold == HDY_FOLD_UNFOLDED ||
      (self->fold == HDY_FOLD_FOLDED && (self->homogeneous[HDY_FOLD_FOLDED][GTK_ORIENTATION_HORIZONTAL] ||
                                         self->homogeneous[HDY_FOLD_FOLDED][GTK_ORIENTATION_VERTICAL] ||
                                ...
                                GParamSpec *pspec)
{
  ...
  switch (prop_id) {
  case PROP_FOLD:
    g_value_set_enum (value, hdy_stackable_box_get_fold (self));
    break;
  case PROP_FOLDED:
    g_value_set_boolean (value, hdy_stackable_box_get_fold (self) == HDY_FOLD_FOLDED);
    break;
...
void
hdy_stackable_box_dispose (HdyStackableBox *self)
{
                               ...
                               gboolean         direct)
{
  ...
  if (self->child_transition.tick_id > 0) {
    ...
    if (((direction < 0 && self->child_transition.can_swipe_back) ||
        (direction > 0 && self->child_transition.can_swipe_forward) ||
         !direct) && self->fold == HDY_FOLD_FOLDED)
...
hdy_stackable_box_class_init (HdyStackableBoxClass *klass)
{
   ...
   *
   * See also: #HdyStackableBox:folded.
   */
  props[PROP_FOLD] =
   ...
   *
   * This is similar to the #HdyStackableBox:fold property but expressed as a
   * #gboolean rather than a #GEnum. This makes it convenient to bind the
   * #HdyStackableBox:fold of a leaflet to any other #gboolean property of other
  ...
  props[PROP_TRANSITION_TYPE] =
    g_param_spec_enum ("transition-type",
                       ...
                       HDY_TYPE_STACKABLE_BOX_TRANSITION_TYPE, HDY_STACKABLE_BOX_TRANSITION_TYPE_NONE,
                       G_PARAM_READWRITE | G_PARAM_EXPLICIT_NOTIFY);
                       ...
                       gboolean           can_unfold)
{
  ...
  self->visible_child = NULL;
  self->fold = can_unfold ? HDY_FOLD_UNFOLDED : HDY_FOLD_FOLDED;
