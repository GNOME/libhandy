                    ...
                    gint                      width)
{
  ...
  if ((self->child_transition.active_direction == GTK_PAN_DIRECTION_RIGHT) == is_rtl) {
    if ((self->child_transition.active_type == HDY_STACKABLE_BOX_TRANSITION_TYPE_OVER ||
         self->child_transition.active_type == HDY_STACKABLE_BOX_TRANSITION_TYPE_SLIDE) &&
        child_info == self->visible_child)
      return width * (1 - self->child_transition.progress) * rtl_multiplier;

    if ((self->child_transition.active_type == HDY_STACKABLE_BOX_TRANSITION_TYPE_UNDER ||
         self->child_transition.active_type == HDY_STACKABLE_BOX_TRANSITION_TYPE_SLIDE) &&
