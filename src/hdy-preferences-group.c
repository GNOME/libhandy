 ...
 * @short_description: A group of preferences to gather in a page.
                                    ...
                                    GParamSpec *pspec)
{
  ...
  case PROP_DESCRIPTION:
    g_value_set_string (value, gtk_label_get_label (priv->description));
    break;
  case PROP_TITLE:
    g_value_set_string (value, gtk_label_get_label (priv->title));
    break;
  default:
                                    ...
                                    GParamSpec   *pspec)
{
  ...
  switch (prop_id) {
  case PROP_DESCRIPTION:
    gtk_label_set_label (priv->description, g_value_get_string (value));
...
hdy_preferences_group_dispose (GObject *object)
{
   ...
   */
  if (priv->description) {
    gtk_widget_destroy (GTK_WIDGET (priv->description));
    priv->description = NULL;
  }
  if (priv->listbox) {
    gtk_widget_destroy (GTK_WIDGET (priv->listbox));
    priv->listbox = NULL;
  }
  if (priv->listbox_box) {
    gtk_widget_destroy (GTK_WIDGET (priv->listbox_box));
    priv->listbox_box = NULL;
  }
  if (priv->title) {
    gtk_widget_destroy (GTK_WIDGET (priv->title));
    priv->title = NULL;
  }
 ...
 *
 * Returns: (transfer none): the title of the group.
 *
 * Since: 0.1.0
 */
const gchar *
