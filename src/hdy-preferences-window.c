 ...
 * grouped into pages. The preferences are searchable.
 *
 * Since: 0.1.0
 */
                       ...
                       HdyPreferencesWindow *self)
{
  ...
  return strstr (hdy_action_row_get_title (row), text) ||
         strstr (hdy_action_row_get_subtitle (row), text);
}
                               ...
                               HdyPreferencesWindow *self)
{
  ...
  group_title = group ? hdy_preferences_group_get_title (group) : NULL;
  ...
  page_title = page ? hdy_preferences_page_get_title (page) : NULL;
  if (g_strcmp0 (page_title, "") == 0)
    page_title = NULL;

  if (group_title && gtk_stack_get_visible_child (priv->pages_title_stack) != GTK_WIDGET (priv->squeezer))
    hdy_action_row_set_subtitle (widget, group_title);
  if (group_title) {
    g_autofree gchar *subtitle = g_strdup_printf ("%s → %s", page_title ? page_title : _("Untitled page"), group_title);
                           ...
                           GdkRectangle         *allocation)
{
  ...
  /* FIXME Tell the fitting box this child should not be displayed. */
  gtk_widget_set_visible (GTK_WIDGET (priv->view_switcher), allocation->width > 360);
}
...
update_pages_switcher_visibility (HdyPreferencesWindow *self)
{
  ...
  gtk_stack_set_visible_child (priv->pages_title_stack,
                               count > 1 ? GTK_WIDGET (priv->squeezer) :
                                           GTK_WIDGET (priv->title));
}
                            ...
                            GtkWidget    *child)
{
  ...
  if (priv->content_stack == NULL)
    ...
    gtk_stack_add_titled (priv->pages_stack, child, NULL, hdy_preferences_page_get_title (HDY_PREFERENCES_PAGE (child)));
...
hdy_preferences_window_class_init (HdyPreferencesWindowClass *klass)
{
  ...
  gtk_widget_class_bind_template_child_private (widget_class, HdyPreferencesWindow, pages_title_stack);
  ...
  gtk_widget_class_bind_template_child_private (widget_class, HdyPreferencesWindow, title);
  gtk_widget_class_bind_template_child_private (widget_class, HdyPreferencesWindow, title_stack);
  gtk_widget_class_bind_template_child_private (widget_class, HdyPreferencesWindow, view_switcher);
  ...
  gtk_widget_class_bind_template_callback (widget_class, gtk_list_box_invalidate_filter);
