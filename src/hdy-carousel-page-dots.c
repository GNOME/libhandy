...
/**
 * SECTION:hdy-carousel-page-dots
 * @short_description: A dots indicator for #HdyCarousel
 * @title: HdyCarouselPageDots
 * @See_also: #HdyCarousel, #HdyCarouselPageLines
 *
 * The #HdyCarouselPageDots widget can be used to show a set of dots for each
 * page of a given #HdyCarousel. Active dot gradually becomes larger and more
 * opaque.
 *
 * # CSS nodes
 *
 * #HdyCarouselPageDots has a single CSS node with name carouselpagedots.
 *
 * Since: 1.0
 */

struct _HdyCarouselPageDots
{
...
G_DEFINE_TYPE_WITH_CODE (HdyCarouselPageDots, hdy_carousel_page_dots, GTK_TYPE_DRAWING_AREA,
...
animation_cb (GtkWidget     *widget,
              ...
              gpointer       user_data)
{
  HdyCarouselPageDots *self = HDY_CAROUSEL_PAGE_DOTS (widget);
  gint64 frame_time;
...
static void
stop_animation (HdyCarouselPageDots *self)
{
...
static void
animate (HdyCarouselPageDots *self,
         gint64               duration)
{
  GdkFrameClock *frame_clock;
  gint64 frame_time;
...
static void
n_pages_changed_cb (HdyCarouselPageDots *self)
{
  animate (self, hdy_carousel_get_reveal_duration (self->carousel));
}

static void
hdy_carousel_page_dots_measure (GtkWidget      *widget,
                                GtkOrientation  orientation,
                                gint            for_size,
                                gint           *minimum,
                                gint           *natural,
                                ...
                                gint           *natural_baseline)
{
  ...
  if (orientation == self->orientation) {
    ...
    size = (2 * DOTS_RADIUS_SELECTED + DOTS_SPACING) * n_pages - DOTS_SPACING;
  } else {
...
static void
hdy_carousel_page_dots_get_preferred_width (GtkWidget *widget,
                                            gint      *minimum_width,
                                            gint      *natural_width)
{
  hdy_carousel_page_dots_measure (widget, GTK_ORIENTATION_HORIZONTAL, -1,
                                  minimum_width, natural_width, NULL, NULL);
}

static void
hdy_carousel_page_dots_get_preferred_height (GtkWidget *widget,
                                             gint      *minimum_height,
                                             gint      *natural_height)
{
  hdy_carousel_page_dots_measure (widget, GTK_ORIENTATION_VERTICAL, -1,
                                  minimum_height, natural_height, NULL, NULL);
}

static gboolean
hdy_carousel_page_dots_draw (GtkWidget *widget,
                             cairo_t   *cr)
{
  HdyCarouselPageDots *self = HDY_CAROUSEL_PAGE_DOTS (widget);
  gint i, n_points;
...
{
  HdyCarouselPageDots *self = HDY_CAROUSEL_PAGE_DOTS (object);

  hdy_carousel_page_dots_set_carousel (self, NULL);

  G_OBJECT_CLASS (hdy_carousel_page_dots_parent_class)->dispose (object);
}

static void
hdy_carousel_page_dots_get_property (GObject    *object,
                                     guint       prop_id,
                                     GValue     *value,
                                     GParamSpec *pspec)
{
  ...
  switch (prop_id) {
  case PROP_CAROUSEL:
    g_value_set_object (value, hdy_carousel_page_dots_get_carousel (self));
    break;
...
static void
hdy_carousel_page_dots_set_property (GObject      *object,
                                     guint         prop_id,
                                     const GValue *value,
                                     GParamSpec   *pspec)
{
  ...
  switch (prop_id) {
  case PROP_CAROUSEL:
    hdy_carousel_page_dots_set_carousel (self, g_value_get_object (value));
    break;
...
static void
hdy_carousel_page_dots_class_init (HdyCarouselPageDotsClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);
  GtkWidgetClass *widget_class = GTK_WIDGET_CLASS (klass);

  object_class->dispose = hdy_carousel_dispose;
  object_class->get_property = hdy_carousel_page_dots_get_property;
  object_class->set_property = hdy_carousel_page_dots_set_property;

  widget_class->get_preferred_width = hdy_carousel_page_dots_get_preferred_width;
  widget_class->get_preferred_height = hdy_carousel_page_dots_get_preferred_height;
  widget_class->draw = hdy_carousel_page_dots_draw;

  /**
   * HdyCarouselPageDots:carousel:
   *
  ...
  gtk_widget_class_set_css_name (widget_class, "carouselpagedots");
}

static void
hdy_carousel_page_dots_init (HdyCarouselPageDots *self)
{
}

/**
 * hdy_carousel_page_dots_new:
 *
 * Create a new #HdyCarouselPageDots widget.
 *
 * Returns: (transfer full): The newly created #HdyCarouselPageDots widget
 *
 * Since: 1.0
 */
GtkWidget *
hdy_carousel_page_dots_new (void)
{
  return g_object_new (HDY_TYPE_CAROUSEL_PAGE_DOTS, NULL);
}

/**
 * hdy_carousel_page_dots_get_carousel:
 * @self: a #HdyCarouselPageDots
 *
 * Get the #HdyCarousel the indicator uses.
 *
 * See: hdy_carousel_page_dots_set_carousel()
 *
...
HdyCarousel *
hdy_carousel_page_dots_get_carousel (HdyCarouselPageDots *self)
{
  g_return_val_if_fail (HDY_IS_CAROUSEL_PAGE_DOTS (self), NULL);

  return self->carousel;
}

/**
 * hdy_carousel_page_dots_set_carousel:
 * @self: a #HdyCarouselPageDots
 ...
 */
void
hdy_carousel_page_dots_set_carousel (HdyCarouselPageDots *self,
                                     HdyCarousel         *carousel)
{
  g_return_if_fail (HDY_IS_CAROUSEL_PAGE_DOTS (self));
