...
static void
get_swipe_area_from_allocation (HdySwipeable *self,
                                GdkRectangle *rect)
{
  rect->x = 0;
  rect->y = 0;
 ...
 * #HdySwipeGroup).
 *
 * If not implemented, the default implementation returns the @self allocation,
                              ...
                              GdkRectangle *rect)
{
  ...
  if (iface->get_swipe_area)
    return (* iface->get_swipe_area) (self, rect);

  return get_swipe_area_from_allocation (self, rect);
}
