...
static void
animate_indicators (HdyPaginator *self,
                    gint64        duration)
{
  GdkFrameClock *frame_clock;
  gint64 frame_time;
...
static gdouble
hdy_paginator_get_distance (HdySwipeable *swipeable)
{
  HdyPaginator *self = HDY_PAGINATOR (swipeable);

  return hdy_paginator_box_get_distance (self->scrolling_box);
}

static void
hdy_paginator_get_range (HdySwipeable *swipeable,
                         gdouble      *lower,
                         gdouble      *upper)
{
  HdyPaginator *self = HDY_PAGINATOR (swipeable);
...
static gdouble *
hdy_paginator_get_snap_points (HdySwipeable *swipeable,
                               gint         *n_snap_points)
{
  HdyPaginator *self = HDY_PAGINATOR (swipeable);
...
static gdouble
hdy_paginator_get_progress (HdySwipeable *swipeable)
{
  HdyPaginator *self = HDY_PAGINATOR (swipeable);

  return hdy_paginator_get_position (self);
}
...
static void
notify_reveal_duration_cb (HdyPaginator *self,
                           GParamSpec   *spec,
                           GObject      *object)
{
...
static void
page_added_cb (HdyPaginator    *self,
               guint            index,
               HdyPaginatorBox *box)
{
  animate_indicators (self, hdy_paginator_get_reveal_duration (self));
}

static void
page_removed_cb (HdyPaginator    *self,
                 guint            index,
                 HdyPaginatorBox *box)
{
  animate_indicators (self, hdy_paginator_get_reveal_duration (self));
                    ...
                    GtkWidget    *widget)
{
  ...
  points = hdy_paginator_box_get_snap_points (self->scrolling_box, &n_points);
  position = hdy_paginator_box_get_position (self->scrolling_box);
  hdy_paginator_box_get_range (self->scrolling_box, &lower, NULL);
                            ...
                            GParamSpec *pspec)
{
  ...
  case PROP_REVEAL_DURATION:
    g_value_set_uint (value, hdy_paginator_get_reveal_duration (self));
    break;
                            ...
                            GParamSpec   *pspec)
{
  ...
  case PROP_REVEAL_DURATION:
    hdy_paginator_set_reveal_duration (self, g_value_get_uint (value));
    break;
...
hdy_paginator_swipeable_init (HdySwipeableInterface *iface)
{
  ...
  iface->get_distance = hdy_paginator_get_distance;
  iface->get_range = hdy_paginator_get_range;
  iface->get_snap_points = hdy_paginator_get_snap_points;
  iface->get_progress = hdy_paginator_get_progress;
  iface->get_cancel_progress = hdy_paginator_get_cancel_progress;
}
...
/**
 * hdy_paginator_get_reveal_duration:
 * @self: a #HdyPaginator
 *
 ...
 */
guint
hdy_paginator_get_reveal_duration (HdyPaginator *self)
{
  g_return_val_if_fail (HDY_IS_PAGINATOR (self), 0);

  return hdy_paginator_box_get_reveal_duration (self->scrolling_box);
}

/**
 * hdy_paginator_set_reveal_duration:
 * @self: a #HdyPaginator
 ...
 */
void
hdy_paginator_set_reveal_duration (HdyPaginator *self,
                                   guint         reveal_duration)
{
  g_return_if_fail (HDY_IS_PAGINATOR (self));

  hdy_paginator_box_set_reveal_duration (self->scrolling_box, reveal_duration);
}
