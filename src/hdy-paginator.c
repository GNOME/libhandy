                            ...
                            GParamSpec *pspec)
{
  ...
  case PROP_MOUSE_DRAG:
    g_value_set_boolean (value, hdy_paginator_get_mouse_drag (self));
    break;
                            ...
                            GParamSpec   *pspec)
{
  ...
  case PROP_MOUSE_DRAG:
    hdy_paginator_set_mouse_drag (self, g_value_get_boolean (value));
    break;
...
hdy_paginator_class_init (HdyPaginatorClass *klass)
{
   ...
   * Since: 0.0.12
   */
  props[PROP_MOUSE_DRAG] =
    g_param_spec_boolean ("mouse-drag",
                          _("Mouse drag"),
 ...
 */
gboolean
hdy_paginator_get_mouse_drag (HdyPaginator *self)
{
  g_return_val_if_fail (HDY_IS_PAGINATOR (self), FALSE);

  return hdy_swipe_tracker_get_mouse_drag (self->tracker);
}

/**
 * hdy_paginator_set_mouse_drag
 * @self: a #HdyPaginator
 * @mouse_drag: whether @self can be dragged with mouse pointer
 *
 * Sets whether @self can be dragged with mouse pointer. If @mouse_drag is
 * %FALSE, dragging is only available on touch.
 *
 * This should usually be %FALSE.
 *
 * Since: 0.0.12
 */
void
hdy_paginator_set_mouse_drag (HdyPaginator *self,
                              gboolean      mouse_drag)
{
  g_return_if_fail (HDY_IS_PAGINATOR (self));
  ...
  hdy_swipe_tracker_set_mouse_drag (self->tracker, mouse_drag);

  g_object_notify_by_pspec (G_OBJECT (self), props[PROP_MOUSE_DRAG]);
}
