...
#include "hdy-header-bar-private.h"
 ...
 * hence allowing it to fill up the whole height, which is needed for
 * #HdyViewSwitcher to be used here.
 */
...
gboolean
_hdy_header_bar_update_window_icon (HdyHeaderBar *self,
                                    GtkWindow    *window)
{
  HdyHeaderBarPrivate *priv = hdy_header_bar_get_instance_private (self);
  GdkPixbuf *pixbuf;
...
void
_hdy_header_bar_update_window_buttons (HdyHeaderBar *self)
{
  HdyHeaderBarPrivate *priv = hdy_header_bar_get_instance_private (self);
    ...
    for (i = 0; i < 2; i++) {
      ...
      for (j = 0; t[j]; j++) {
        ...
        if (strcmp (t[j], "icon") == 0 &&
            ...
            if (!_hdy_header_bar_update_window_icon (self, window))
              {
...
gboolean
_hdy_header_bar_shows_app_menu (HdyHeaderBar *self)
{
  HdyHeaderBarPrivate *priv = hdy_header_bar_get_instance_private (self);
  GtkWindow *window;
  gchar *layout_desc;
                         ...
                         gint           *natural_size)
{
  ...
  if (orientation == GTK_ORIENTATION_HORIZONTAL) {
    ...
    minimum = 2 * MAX (start_min + n_start_children * priv->spacing,
                       end_min + n_end_children * priv->spacing);
    natural = 2 * MAX (start_nat + n_start_children * priv->spacing,
                       end_nat + n_end_children * priv->spacing);
    } else {
      minimum = start_min + n_start_children * priv->spacing +
                end_min + n_end_children * priv->spacing;
      natural = start_nat + n_start_children * priv->spacing +
                end_nat + n_end_children * priv->spacing;
    }
                              ...
                              GtkAllocation *allocation)
{
  ...
  if (gtk_progress_tracker_get_state (&priv->tracker) != GTK_PROGRESS_STATE_AFTER)
  {
    ...
    gint max = MAX (side[0], side[1]);
    g_message ("a side[0] = %d, side[1] = %d, max = %d, t = %lf", side[0], side[1], max, t);
    if (priv->centering_policy == HDY_CENTERING_POLICY_STRICT) {
      side[0] = LERP (max, side[0], t);
      side[1] = LERP (max, side[1], t);
    } else {
      side[0] = LERP (side[0], max, t);
      side[1] = LERP (side[1], max, t);
    }
    g_message ("b side[0] = %d, side[1] = %d, max = %d, t = %lf", side[0], side[1], max, t);
  }
  else if (priv->centering_policy == HDY_CENTERING_POLICY_STRICT)
    side[0] = side[1] = MAX (side[0], side[1]);
...
hdy_header_bar_realize (GtkWidget *widget)
{
  ...
  g_signal_connect_swapped (settings, "notify::gtk-shell-shows-app-menu",
                            G_CALLBACK (_hdy_header_bar_update_window_buttons), widget);
  g_signal_connect_swapped (settings, "notify::gtk-decoration-layout",
                            G_CALLBACK (_hdy_header_bar_update_window_buttons), widget);
  _hdy_header_bar_update_window_buttons (HDY_HEADER_BAR (widget));
}
...
hdy_header_bar_unrealize (GtkWidget *widget)
{
  ...
  g_signal_handlers_disconnect_by_func (settings, _hdy_header_bar_update_window_buttons, widget);
                      ...
                      gpointer             data)
{
  ...
  if (event->changed_mask & (GDK_WINDOW_STATE_FULLSCREEN |
    ...
    _hdy_header_bar_update_window_buttons (self);
