...
} HdySwipeGroupPrivate;

static void hdy_swipe_group_buildable_init (GtkBuildableIface *iface);
static gboolean hdy_swipe_group_buildable_custom_tag_start (GtkBuildable  *buildable,
                                                             GtkBuilder    *builder,
                                                             GObject       *child,
                                                             const gchar   *tagname,
                                                             GMarkupParser *parser,
                                                             gpointer      *data);
static void hdy_swipe_group_buildable_custom_finished (GtkBuildable *buildable,
                                                        GtkBuilder   *builder,
                                                        GObject      *child,
                                                        const gchar  *tagname,
                                                        gpointer      user_data);
...
{
  HdySwipeGroupPrivate *priv;
  GSList *swipeables;

  priv = hdy_swipe_group_get_instance_private (self);

  for (swipeables = priv->swipeables; swipeables != NULL; swipeables = swipeables->next)
                     ...
                     HdySwipeable  *swipeable)
{
  ...
  priv = hdy_swipe_group_get_instance_private (self);
  priv->swipeables = g_slist_remove (priv->swipeables, swipeable);
...
switch_child_cb (HdySwipeGroup *self,
                 ...
                 HdySwipeable  *swipeable)
{
  HdySwipeGroupPrivate *priv = hdy_swipe_group_get_instance_private (self);
  GSList *swipeables;

  if (swipeable == priv->current)
    return;
                ...
                HdySwipeable  *swipeable)
{
  ...
  if (priv->current != NULL && priv->current != swipeable)
    return;

  priv->current = swipeable;
                               ...
                               HdySwipeable  *swipeable)
{
  ...
  priv->swipeables = g_slist_prepend (priv->swipeables, swipeable);
 ...
 *   swipeable. The list is owned by libhandy and should not be modified.
 **/
GSList *
...
hdy_swipe_group_dispose (GObject *object)
{
  ...
  g_slist_free_full (priv->swipeables, (GDestroyNotify) g_object_unref);
  priv->swipeables = NULL;
