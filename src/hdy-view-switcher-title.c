...
update_subtitle_label (HdyViewSwitcherTitle *self)
{
  ...
  gtk_widget_set_visible (GTK_WIDGET (priv->subtitle_label), priv->has_subtitle || (subtitle && subtitle[0]));
}
...
hdy_view_switcher_title_init (HdyViewSwitcherTitle *self)
{
   ...
   */
  priv->policy = HDY_VIEW_SWITCHER_POLICY_WIDE;
...
hdy_view_switcher_title_get_view_switcher_enabled (HdyViewSwitcherTitle *self)
{
  ...
  return hdy_squeezer_get_child_enabled (priv->squeezer, GTK_WIDGET (priv->view_switcher));
}
                                                   ...
                                                   gboolean              enabled)
{
  ...
  if (hdy_squeezer_get_child_enabled (priv->squeezer, GTK_WIDGET (priv->view_switcher)) == enabled)
    return;

  hdy_squeezer_set_child_enabled (priv->squeezer, GTK_WIDGET (priv->view_switcher), enabled);
