 ...
 * |[<!-- language="C" -->
 * static GdkPixbuf *
 * image_load_func (gint size, gint scale_factor, gpointer user_data)
 * {
 *   g_autofree gchar *file = NULL;
 *   gint actual_width, actual_height, width, height;
 *
 *   file = gtk_file_chooser_get_filename ("avatar.png");
 *
 *   gdk_pixbuf_get_file_info (file, &width, &height);
 *   actual_width = (width <= height) ? size : -1;
 *   actual_height = (width >= height) ? size : -1;
 *
 *   return gdk_pixbuf_new_from_file_at_scale (file,
 *                                             actual_width * scale_factor,
 *                                             actual_height * scale_factor,
 *                                             TRUE,
 *                                             NULL);
 * }
 * ]|
...
update_custom_image (HdyAvatar *self)
{
  ...
  if (priv->load_image_func_target != NULL) {
    ...
    priv->round_image = round_image (pixbuf, (gdouble) size);
