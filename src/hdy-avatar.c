 ...
 *   gint width, height;
 *
 *   file = gtk_file_chooser_get_filename ("avatar.png");
 *
 *   gdk_pixbuf_get_file_info (file, &width, &height);
 *
 *   return gdk_pixbuf_new_from_file_at_scale (file,
 *                                             (width <= height) ? size : -1;
 *                                             (width >= height) ? size : -1;
 *                                             TRUE,
 *                                             NULL);
...
extract_initials_from_text (const gchar *text)
{
  ...
  g_autofree gchar *normalized = g_utf8_normalize (g_strstrip (p),
                                                   -1,
...
update_custom_image (HdyAvatar *self)
{
  ...
  if (priv->load_image_func != NULL) {
    ...
    pixbuf = priv->load_image_func (size * scale_factor,
                                    priv->load_image_func_target);
    priv->round_image = round_image (pixbuf, (gdouble) size * scale_factor);
    cairo_surface_set_device_scale (priv->round_image,
                                    scale_factor,
                                    scale_factor);
  }
...
{
  HdyAvatarPrivate *priv = hdy_avatar_get_instance_private (HDY_AVATAR (self));
...
ensure_pango_layout (HdyAvatar *self)
{
  ...
  if (priv->layout == NULL && priv->text != NULL && strlen (priv->text)) {
  ...
  if (priv->round_image) {
    ...
    if (priv->show_initials && priv->layout != NULL) {
      ...
      gtk_style_context_get_color (context,
                                   gtk_style_context_get_state (context),
                                   &color);
      pixbuf = gtk_icon_info_load_symbolic (icon,
                                            &color,
                                            NULL,
                                            NULL,
                                            NULL,
                                            ...
                                            &error);
      if (error != NULL) {
        g_warning ("Failed to load avatar-default-symbolic: %s", error->message);
      } else {
...
hdy_avatar_init (HdyAvatar *self)
{
  ...
  g_signal_connect (self,
                    "notify::scale-factor",
                    G_CALLBACK (update_custom_image),
                    NULL);
  g_signal_connect (self,
                    "size-allocate",
                    G_CALLBACK (update_custom_image),
 ...
 * @self: a #HdyAvatar
 * @load_image: (closure user_data) (allow-none): callback to set
 * a custom image
 * @user_data: user data passed to @load_image
 * @destroy: destroy notifier for @user_data
 *
 * A callback which is called when the custom image need to be reloaded for
