...
static void
set_font_size (HdyAvatar *self, gint size)
{
  HdyAvatarPrivate *priv;
  GtkStyleContext *context;
  ...
  pango_font_description_set_absolute_size (font_desc, CLAMP (new_font_size, 0, max_size) * PANGO_SCALE);
                 ...
                 cairo_t   *cr)
{
  ...
  x = (gdouble)(width - size) / 2.0;
  y = (gdouble)(height - size) / 2.0;
  ...
  if (priv->round_image) {
    ...
    if (priv->show_initials && priv->layout != NULL && priv->text != NULL && strlen (priv->text) != 0) {
