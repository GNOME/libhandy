 ...
 * The color is picked based on the hash of the #HdyAvatar:text. If #HdyAvatar:show-initials is set to %FALSE `avatar-default-symbolic` in place of the initials.
 *
...
generate_user_picture (const gchar *text, gint size, gboolean show_initials)
{
  ...
  if (show_initials) {
    ...
    font = g_strdup_printf ("Cantarell Bold %d", (int)ceil (size / 4));
 ...
 */
GtkWidget *
hdy_avatar_new (GdkPixbuf *image, gint size, const gchar *text, gboolean show_initials)
{
 ...
 */
void
hdy_avatar_set_text (HdyAvatar *self, const gchar *text)
{
  HdyAvatarPrivate *priv;
  g_return_if_fail (HDY_IS_AVATAR (self));
