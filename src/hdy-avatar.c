...
round_image (GdkPixbuf *pixbuf, gdouble size)
{
...
set_font_size (HdyAvatar *self)
{
  HdyAvatarPrivate *priv;
  GtkStyleContext *context;
  const PangoFontDescription *const_font_desc;
  ...
  if (width > size || height > size) {
    const_font_desc = pango_layout_get_font_description (priv->layout);
    font_desc = pango_font_description_copy (const_font_desc);
    pango_font_description_set_absolute_size (font_desc, size/3 * PANGO_SCALE);
    pango_layout_set_font_description (priv->layout, font_desc);
    pango_font_description_free (font_desc);
  }
}
  ...
  if (priv->round_image) {
    ...
    if (priv->show_initials && priv->layout != NULL) {
      ...
      gtk_render_layout (context,
                         cr,
                         ((gdouble)size - ((gdouble)width)) / 2.0,
                         ((gdouble)size - ((gdouble)height)) / 2.0,
                         priv->layout);
    } else {
