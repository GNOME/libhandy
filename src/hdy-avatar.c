...
static cairo_surface_t *
create_fallback (const gchar *name, gint size, gboolean show_initials)
{
  g_autoptr (GdkPixbuf) pixbuf = NULL;
  cairo_surface_t *surface;

  surface = generate_user_picture (name, size, show_initials);

  pixbuf = gdk_pixbuf_get_from_surface (surface, 0, 0, size, size);
  cairo_surface_destroy (surface);

  return round_image (pixbuf);
}
                         ...
                         GParamSpec *pspec)
{
  ...
  switch (property_id) {
  case PROP_IMAGE:
    g_value_set_object (value, hdy_avatar_get_image (self));
    break;
...
hdy_avatar_draw (GtkWidget *widget, cairo_t *cr)
{
  ...
  if (priv->cache != NULL) {
    /* Don't do anything if we already have a cached avatar */
  } else if (priv->image != NULL) {
    scale_factor = gtk_widget_get_scale_factor (widget);
    priv->cache = round_image (priv->image);
    cairo_surface_set_device_scale (priv->cache, scale_factor, scale_factor);
  } else {
    scale_factor = gtk_widget_get_scale_factor (widget);
    priv->cache = create_fallback (priv->name, priv->size * scale_factor, priv->show_initials);
    cairo_surface_set_device_scale (priv->cache, scale_factor, scale_factor);
  }
 ...
 * @name: The name used to generate the color and initials if @show_initials is TRUE
 * @show_initials: whether to show the initials or the fallback icon on top of the color
 *
...
GdkPixbuf *
hdy_avatar_get_image (HdyAvatar *self)
{
  HdyAvatarPrivate *priv;
  g_return_val_if_fail (HDY_IS_AVATAR (self), NULL);
