...
static GtkWindow *
get_window (HdyWindowHandle *self)
{
...
popup_menu_detach (GtkWidget *widget,
                   GtkMenu   *menu)
{
  HDY_WINDOW_HANDLE (widget)->fallback_menu = NULL;
}

static void
restore_window_cb (GtkMenuItem     *menuitem,
                   HdyWindowHandle *self)
{
...
static gboolean
titlebar_action (HdyWindowHandle *self,
...
static gboolean
hdy_window_handle_event (GtkWidget *widget,
                         GdkEvent  *event)
{
  HdyWindowHandle *self = HDY_WINDOW_HANDLE (widget);
  GdkEventType type;
  GdkEventSequence *seq;
  gboolean retval;
  GtkEventController *controller;

  if (!self->multipress_gesture)
  ...
  controller = GTK_EVENT_CONTROLLER (self->multipress_gesture);
  retval = gtk_event_controller_handle_event (controller, event);
   ...
   */
  if (gtk_gesture_get_sequence_state (GTK_GESTURE (controller), seq) == GTK_EVENT_SEQUENCE_DENIED)
...
hdy_window_handle_class_init (HdyWindowHandleClass *klass)
{
  ...
  widget_class->event = hdy_window_handle_event;
