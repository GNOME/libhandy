 ...
 * @Title: HdyStack
 *
 * The #HdyStack widget can display its children like a #GtkBox does or
 * like a #HdyStack does, adapting to size changes by switching between
 * the two modes.
 *
 ...
 * @HDY_STACK_TRANSITION_TYPE_NONE: No transition
 * @HDY_STACK_TRANSITION_TYPE_SLIDE: Slide from left, right, up or down according to the orientation, text direction and the children order
 * @HDY_STACK_TRANSITION_TYPE_OVER: Cover the old page or uncover the new page, sliding from or towards the end according to orientation, text direction and children order
 * @HDY_STACK_TRANSITION_TYPE_UNDER: Uncover the new page or cover the old page, sliding from or towards the start according to orientation, text direction and children order
 *
 * This enumeration value describes the possible transitions between modes and
 * children in a #HdyStack widget.
 *
...
typedef struct _HdyStackChildInfo HdyStackChildInfo;

struct _HdyStackChildInfo
{
  GtkWidget *widget;
  gchar *name;
...
typedef struct
{
   ...
   * draw children for RTL languages on a horizontal stack.
   */
  GList *children_reversed;
  HdyStackChildInfo *visible_child;
  HdyStackChildInfo *last_visible_child;
  ...
  HdyStackTransitionType transition_type;

  HdySwipeTracker *tracker;

  /* Transition variables. */
  struct {
    HdyStackTransitionType type;
    guint duration;
    ...
    HdyStackTransitionType active_type;
...
static void hdy_stack_buildable_init (GtkBuildableIface  *iface);
static void hdy_stack_swipeable_init (HdySwipeableInterface *iface);

G_DEFINE_TYPE_WITH_CODE (HdyStack, hdy_stack, GTK_TYPE_CONTAINER,
                         G_ADD_PRIVATE (HdyStack)
                         G_IMPLEMENT_INTERFACE (GTK_TYPE_ORIENTABLE, NULL)
                         G_IMPLEMENT_INTERFACE (GTK_TYPE_BUILDABLE, hdy_stack_buildable_init)
                         G_IMPLEMENT_INTERFACE (HDY_TYPE_SWIPEABLE, hdy_stack_swipeable_init))

static void
free_child_info (HdyStackChildInfo *child_info)
{
  g_free (child_info->name);
  g_free (child_info);
}

static HdyStackChildInfo *
find_child_info_for_widget (HdyStack  *self,
                            GtkWidget *widget)
{
  HdyStackPrivate *priv = hdy_stack_get_instance_private (self);
  GList *children;
  HdyStackChildInfo *child_info;
...
static HdyStackChildInfo *
find_child_info_for_name (HdyStack  *self,
                          const gchar *name)
{
  HdyStackPrivate *priv = hdy_stack_get_instance_private (self);
  GList *children;
...
static GList *
get_directed_children (HdyStack *self)
{
  HdyStackPrivate *priv = hdy_stack_get_instance_private (self);
...
is_window_moving_transition (HdyStack *self)
{
  HdyStackPrivate *priv = hdy_stack_get_instance_private (self);
  ...
  case HDY_STACK_TRANSITION_TYPE_NONE:
    return FALSE;
  case HDY_STACK_TRANSITION_TYPE_SLIDE:
    return TRUE;
  case HDY_STACK_TRANSITION_TYPE_OVER:
    return direction == GTK_PAN_DIRECTION_UP || direction == left_or_right;
  case HDY_STACK_TRANSITION_TYPE_UNDER:
...
old and new child */
static inline gboolean
is_direction_dependent_transition (HdyStackTransitionType transition_type)
{
  return (transition_type == HDY_STACK_TRANSITION_TYPE_SLIDE ||
          transition_type == HDY_STACK_TRANSITION_TYPE_OVER ||
          transition_type == HDY_STACK_TRANSITION_TYPE_UNDER);
}

static GtkPanDirection
get_pan_direction (HdyStack *self,
...
static gint
get_bin_window_x (HdyStack          *self,
...
static gint
get_bin_window_y (HdyStack          *self,
                  const GtkAllocation *allocation)
{
  HdyStackPrivate *priv = hdy_stack_get_instance_private (self);
  int y = 0;
...
static void
move_resize_bin_window (HdyStack    *self,
...
static void
hdy_stack_child_progress_updated (HdyStack *self)
{
  HdyStackPrivate *priv = hdy_stack_get_instance_private (self);
...
static gboolean
hdy_stack_transition_cb (GtkWidget     *widget,
                         GdkFrameClock *frame_clock,
                         gpointer       user_data)
{
  HdyStack *self = HDY_STACK (widget);
  HdyStackPrivate *priv = hdy_stack_get_instance_private (self);
  gdouble progress;
  ...
  hdy_stack_child_progress_updated (self);
...
static void
hdy_stack_schedule_child_ticks (HdyStack *self)
{
  HdyStackPrivate *priv = hdy_stack_get_instance_private (self);

  if (priv->transition.tick_id == 0) {
    priv->transition.tick_id =
      gtk_widget_add_tick_callback (GTK_WIDGET (self), hdy_stack_transition_cb, self, NULL);
    if (!priv->transition.is_gesture_active)
      g_object_notify_by_pspec (G_OBJECT (self), props[PROP_TRANSITION_RUNNING]);
  }
}
...
static void
hdy_stack_unschedule_child_ticks (HdyStack *self)
{
  HdyStackPrivate *priv = hdy_stack_get_instance_private (self);
...
static void
hdy_stack_start_transition (HdyStack               *self,
                            HdyStackTransitionType  transition_type,
                            ...
                            GtkPanDirection         transition_direction)
{
  ...
  if (gtk_widget_get_mapped (widget) &&
      ...
      transition_type != HDY_STACK_TRANSITION_TYPE_NONE &&
    ...
    if (!priv->transition.is_gesture_active) {
      hdy_stack_schedule_child_ticks (self);
    ...
    hdy_stack_unschedule_child_ticks (self);
    priv->transition.active_type = HDY_STACK_TRANSITION_TYPE_NONE;
    gtk_progress_tracker_finish (&priv->transition.tracker);
  }
...
static void
set_visible_child_info (HdyStack               *self,
                        HdyStackChildInfo      *new_visible_child,
                        HdyStackTransitionType  transition_type,
                        guint                   transition_duration,
                        gboolean                emit_switch_child)
{
  ...
  /* hdy_stack_accessible_update_visible_child (stack, */
  ...
  hdy_stack_start_transition (self, transition_type, transition_duration, transition_direction);
...
/**
 * hdy_stack_set_homogeneous:
 * @self: a #HdyStack
 * @orientation: the orientation
 * @homogeneous: %TRUE to make @self homogeneous
 *
 * Sets the #HdyStack to be homogeneous or not for the given orientation.
 * If it is homogeneous, the #HdyStack will request the same
 ...
 */
void
hdy_stack_set_homogeneous (HdyStack       *self,
                           GtkOrientation  orientation,
                           gboolean        homogeneous)
{
  HdyStackPrivate *priv;

  g_return_if_fail (HDY_IS_STACK (self));

  priv = hdy_stack_get_instance_private (self);
...
/**
 * hdy_stack_get_homogeneous:
 * @self: a #HdyStack
 * @orientation: the orientation
 *
 * Gets whether @self is homogeneous for the given orientation.
 * See hdy_stack_set_homogeneous().
 *
 * Returns: whether @self is homogeneous for the given orientation.
 */
gboolean
hdy_stack_get_homogeneous (HdyStack       *self,
                           GtkOrientation  orientation)
{
  HdyStackPrivate *priv;

  g_return_val_if_fail (HDY_IS_STACK (self), FALSE);
 ...
 */
HdyStackTransitionType
hdy_stack_get_transition_type (HdyStack *self)
{
  HdyStackPrivate *priv;

  g_return_val_if_fail (HDY_IS_STACK (self), HDY_STACK_TRANSITION_TYPE_NONE);
 ...
 */
void
hdy_stack_set_transition_type (HdyStack               *self,
...
/**
 * hdy_stack_get_transition_duration:
 * @self: a #HdyStack
 *
 ...
 */
guint
hdy_stack_get_transition_duration (HdyStack *self)
{
  HdyStackPrivate *priv;

  g_return_val_if_fail (HDY_IS_STACK (self), 0);
...
/**
 * hdy_stack_set_transition_duration:
 * @self: a #HdyStack
 ...
 */
void
hdy_stack_set_transition_duration (HdyStack *self,
                                   guint     duration)
{
  HdyStackPrivate *priv;
 ...
 */
GtkWidget *
hdy_stack_get_visible_child (HdyStack *self)
{
  HdyStackPrivate *priv;

  g_return_val_if_fail (HDY_IS_STACK (self), NULL);
...
void
hdy_stack_set_visible_child (HdyStack  *self,
...
const gchar *
hdy_stack_get_visible_child_name (HdyStack *self)
{
  HdyStackPrivate *priv;
...
void
hdy_stack_set_visible_child_name (HdyStack    *self,
                                  const gchar *name)
{
  HdyStackPrivate *priv;
  HdyStackChildInfo *child_info;
...
/**
 * hdy_stack_get_transition_running:
 * @self: a #HdyStack
 *
 ...
 */
gboolean
hdy_stack_get_transition_running (HdyStack *self)
{
  HdyStackPrivate *priv;
...
/**
 * hdy_stack_set_interpolate_size:
 * @self: a #HdyStack
 * @interpolate_size: the new value
 *
 * Sets whether or not @self will interpolate its size when
 * changing the visible child. If the #HdyStack:interpolate-size
 * property is set to %TRUE, @stack will interpolate its size between
 * the current one and the one it'll take after changing the
 * visible child, according to the set transition duration.
 */
void
hdy_stack_set_interpolate_size (HdyStack *self,
                                gboolean  interpolate_size)
{
  HdyStackPrivate *priv;
...
/**
 * hdy_stack_get_interpolate_size:
 * @self: a #HdyStack
 *
 * Returns wether the #HdyStack is set up to interpolate between
 * the sizes of children on page switch.
 *
 * Returns: %TRUE if child sizes are interpolated
 */
gboolean
hdy_stack_get_interpolate_size (HdyStack *self)
{
  HdyStackPrivate *priv;
 ...
 */
void
hdy_stack_set_can_swipe_back (HdyStack *self,
                              gboolean  can_swipe_back)
{
  HdyStackPrivate *priv;
...
/**
 * hdy_stack_get_can_swipe_back
 * @self: a #HdyStack
 *
 * Returns whether the #HdyStack allows swiping to the previous child.
 *
 ...
 */
gboolean
hdy_stack_get_can_swipe_back (HdyStack *self)
{
  HdyStackPrivate *priv;
...
/**
 * hdy_stack_set_can_swipe_forward:
 * @self: a #HdyStack
 ...
 */
void
hdy_stack_set_can_swipe_forward (HdyStack *self,
                                 gboolean  can_swipe_forward)
{
  HdyStackPrivate *priv;
...
/**
 * hdy_stack_get_can_swipe_forward
 * @self: a #HdyStack
 *
 * Returns whether the #HdyStack allows swiping to the next child.
 *
 * Returns: %TRUE if back swipe is enabled.
 *
 * Since: 0.0.12
 */
gboolean
hdy_stack_get_can_swipe_forward (HdyStack *self)
{
  HdyStackPrivate *priv;
 ...
 */
static void
hdy_stack_measure (GtkWidget      *widget,
                   GtkOrientation  orientation,
                   int             for_size,
                   int            *minimum,
                   int            *natural,
...
static void
hdy_stack_get_preferred_width (GtkWidget *widget,
                               gint      *minimum_width,
                               gint      *natural_width)
{
  hdy_stack_measure (widget, GTK_ORIENTATION_HORIZONTAL, -1,
                       minimum_width, natural_width, NULL, NULL);
}

static void
hdy_stack_get_preferred_height (GtkWidget *widget,
                                gint      *minimum_height,
                                gint      *natural_height)
{
  hdy_stack_measure (widget, GTK_ORIENTATION_VERTICAL, -1,
                       minimum_height, natural_height, NULL, NULL);
}

static void
hdy_stack_get_preferred_width_for_height (GtkWidget *widget,
                                          gint       height,
                                          gint      *minimum_width,
                                          gint      *natural_width)
{
  hdy_stack_measure (widget, GTK_ORIENTATION_HORIZONTAL, height,
                       minimum_width, natural_width, NULL, NULL);
}

static void
hdy_stack_get_preferred_height_for_width (GtkWidget *widget,
                                          gint       width,
                                          gint      *minimum_height,
                                          gint      *natural_height)
{
  hdy_stack_measure (widget, GTK_ORIENTATION_VERTICAL, width,
                       minimum_height, natural_height, NULL, NULL);
}

static void
hdy_stack_size_allocate_children (GtkWidget     *widget,
                                  GtkAllocation *allocation)
{
  HdyStack *self = HDY_STACK (widget);
  HdyStackPrivate *priv = hdy_stack_get_instance_private (self);
  GList *directed_children, *children;
  HdyStackChildInfo *child_info, *visible_child;
...
static void
hdy_stack_size_allocate (GtkWidget     *widget,
                         GtkAllocation *allocation)
{
  HdyStack *self = HDY_STACK (widget);
  HdyStackPrivate *priv = hdy_stack_get_instance_private (self);
  ...
  hdy_stack_size_allocate_children (widget, allocation);
...
static void
hdy_stack_draw_under (GtkWidget *widget,
                      cairo_t   *cr)
{
  HdyStack *self = HDY_STACK (widget);
  HdyStackPrivate *priv = hdy_stack_get_instance_private (self);
...
static void
hdy_stack_draw_over (GtkWidget *widget,
                     cairo_t   *cr)
{
  HdyStack *self = HDY_STACK (widget);
  HdyStackPrivate *priv = hdy_stack_get_instance_private (self);
...
static void
hdy_stack_draw_slide (GtkWidget *widget,
                      cairo_t   *cr)
{
  HdyStack *self = HDY_STACK (widget);
  HdyStackPrivate *priv = hdy_stack_get_instance_private (self);
...
static void
hdy_stack_draw_over_or_under (GtkWidget *widget,
                              cairo_t   *cr)
{
  HdyStack *self = HDY_STACK (widget);
  HdyStackPrivate *priv = hdy_stack_get_instance_private (self);
...
static gboolean
hdy_stack_draw (GtkWidget *widget,
                cairo_t   *cr)
{
  HdyStack *self = HDY_STACK (widget);
  HdyStackPrivate *priv = hdy_stack_get_instance_private (self);
  ...
  if (priv->visible_child) {
    if ((priv->transition.is_gesture_active &&
        priv->transition_type != HDY_STACK_TRANSITION_TYPE_NONE) ||
        gtk_progress_tracker_get_state (&priv->transition.tracker) != GTK_PROGRESS_STATE_AFTER) {
      if (priv->transition.last_visible_surface == NULL &&
          priv->last_visible_child != NULL) {
      ...
      switch (priv->transition.active_type) {
      case HDY_STACK_TRANSITION_TYPE_SLIDE:
        hdy_stack_draw_slide (widget, cr);
        break;
      case HDY_STACK_TRANSITION_TYPE_OVER:
      case HDY_STACK_TRANSITION_TYPE_UNDER:
        hdy_stack_draw_over_or_under (widget, cr);
        break;
      case HDY_STACK_TRANSITION_TYPE_NONE:
      default:
        g_assert_not_reached ();
...
static void
update_tracker_orientation (HdyStack *self)
{
  HdyStackPrivate *priv = hdy_stack_get_instance_private (self);
  gboolean reverse;
...
static void
hdy_stack_direction_changed (GtkWidget        *widget,
...
static void
hdy_stack_child_visibility_notify_cb (GObject    *obj,
                                      GParamSpec *pspec,
                                      gpointer    user_data)
{
  HdyStack *self = HDY_STACK (user_data);
  HdyStackPrivate *priv = hdy_stack_get_instance_private (self);
...
static void
hdy_stack_add (GtkContainer *container,
               GtkWidget    *widget)
{
  HdyStack *self;
  HdyStackPrivate *priv;
  ...
  child_info = g_new0 (HdyStackChildInfo, 1);
  ...
  g_signal_connect (widget, "notify::visible",
                    G_CALLBACK (hdy_stack_child_visibility_notify_cb), self);

  if (hdy_stack_get_visible_child (self) == NULL &&
...
static void
hdy_stack_remove (GtkContainer *container,
                  GtkWidget    *widget)
{
  HdyStack *self = HDY_STACK (container);
  HdyStackPrivate *priv = hdy_stack_get_instance_private (self);
  HdyStackChildInfo *child_info;
  gboolean contains_child;
  ...
  if (hdy_stack_get_visible_child (self) == widget)
    set_visible_child_info (self, NULL, priv->transition_type,
                            priv->transition.duration, TRUE);
...
static void
hdy_stack_forall (GtkContainer *container,
                  ...
                  gpointer      callback_data)
{
   ...
   * looping through it, for example by calling hdy_stack_remove() on all
   * children when destroying the HdyStack_private_offset.
   */
...
static void
hdy_stack_get_property (GObject    *object,
                        guint       prop_id,
                        GValue     *value,
                        GParamSpec *pspec)
{
  HdyStack *self = HDY_STACK (object);
  HdyStackPrivate *priv = hdy_stack_get_instance_private (self);

  switch (prop_id) {
  case PROP_HHOMOGENEOUS:
    g_value_set_boolean (value, hdy_stack_get_homogeneous (self, GTK_ORIENTATION_HORIZONTAL));
    break;
  case PROP_VHOMOGENEOUS:
    g_value_set_boolean (value, hdy_stack_get_homogeneous (self, GTK_ORIENTATION_VERTICAL));
    break;
  case PROP_VISIBLE_CHILD:
    g_value_set_object (value, hdy_stack_get_visible_child (self));
    break;
  case PROP_VISIBLE_CHILD_NAME:
    g_value_set_string (value, hdy_stack_get_visible_child_name (self));
    break;
  case PROP_TRANSITION_TYPE:
    g_value_set_enum (value, hdy_stack_get_transition_type (self));
    break;
  case PROP_TRANSITION_DURATION:
    g_value_set_uint (value, hdy_stack_get_transition_duration (self));
    break;
  case PROP_TRANSITION_RUNNING:
    g_value_set_boolean (value, hdy_stack_get_transition_running (self));
    break;
  case PROP_INTERPOLATE_SIZE:
    g_value_set_boolean (value, hdy_stack_get_interpolate_size (self));
    break;
  case PROP_CAN_SWIPE_BACK:
    g_value_set_boolean (value, hdy_stack_get_can_swipe_back (self));
    break;
  case PROP_CAN_SWIPE_FORWARD:
    g_value_set_boolean (value, hdy_stack_get_can_swipe_forward (self));
    break;
...
static void
hdy_stack_set_property (GObject      *object,
                        guint         prop_id,
                        const GValue *value,
                        GParamSpec   *pspec)
{
  ...
  switch (prop_id) {
  case PROP_HHOMOGENEOUS:
    hdy_stack_set_homogeneous (self, GTK_ORIENTATION_HORIZONTAL, g_value_get_boolean (value));
    break;
  case PROP_VHOMOGENEOUS:
    hdy_stack_set_homogeneous (self, GTK_ORIENTATION_VERTICAL, g_value_get_boolean (value));
    break;
  case PROP_VISIBLE_CHILD:
    hdy_stack_set_visible_child (self, g_value_get_object (value));
    break;
  case PROP_VISIBLE_CHILD_NAME:
    hdy_stack_set_visible_child_name (self, g_value_get_string (value));
    break;
  case PROP_TRANSITION_TYPE:
    hdy_stack_set_transition_type (self, g_value_get_enum (value));
    break;
  case PROP_TRANSITION_DURATION:
    hdy_stack_set_transition_duration (self, g_value_get_uint (value));
    break;
  case PROP_INTERPOLATE_SIZE:
    hdy_stack_set_interpolate_size (self, g_value_get_boolean (value));
    break;
  case PROP_CAN_SWIPE_BACK:
    hdy_stack_set_can_swipe_back (self, g_value_get_boolean (value));
    break;
  case PROP_CAN_SWIPE_FORWARD:
    hdy_stack_set_can_swipe_forward (self, g_value_get_boolean (value));
    break;
  case PROP_ORIENTATION:
    {
...
static void
hdy_stack_dispose (GObject *object)
{
  HdyStack *self = HDY_STACK (object);
  HdyStackPrivate *priv = hdy_stack_get_instance_private (self);
  ...
  G_OBJECT_CLASS (hdy_stack_parent_class)->dispose (object);
}

static void
hdy_stack_finalize (GObject *object)
{
  HdyStack *self = HDY_STACK (object);
  HdyStackPrivate *priv = hdy_stack_get_instance_private (self);
  ...
  G_OBJECT_CLASS (hdy_stack_parent_class)->finalize (object);
}

static void
hdy_stack_get_child_property (GtkContainer *container,
                              GtkWidget    *widget,
                              guint         property_id,
                              GValue       *value,
                              GParamSpec   *pspec)
...
static void
hdy_stack_set_child_property (GtkContainer *container,
                              GtkWidget    *widget,
                              guint         property_id,
                              const GValue *value,
                              GParamSpec   *pspec)
  ...
  case CHILD_PROP_NAME:
    ...
    for (children = priv->children; children; children = children->next) {
      ...
      if (g_strcmp0 (child_info2->name, name) == 0) {
        g_warning ("Duplicate child name in HdyStack: %s", name);
...
static void
hdy_stack_realize (GtkWidget *widget)
{
  HdyStack *self = HDY_STACK (widget);
  HdyStackPrivate *priv = hdy_stack_get_instance_private (self);
  GtkAllocation allocation;
...
static void
hdy_stack_unrealize (GtkWidget *widget)
{
  HdyStack *self = HDY_STACK (widget);
  HdyStackPrivate *priv = hdy_stack_get_instance_private (self);
  ...
  GTK_WIDGET_CLASS (hdy_stack_parent_class)->unrealize (widget);
}

static void
hdy_stack_map (GtkWidget *widget)
{
  HdyStack *self = HDY_STACK (widget);
  HdyStackPrivate *priv = hdy_stack_get_instance_private (self);

  GTK_WIDGET_CLASS (hdy_stack_parent_class)->map (widget);

  gdk_window_show (priv->view_window);
}

static void
hdy_stack_unmap (GtkWidget *widget)
{
  HdyStack *self = HDY_STACK (widget);
  HdyStackPrivate *priv = hdy_stack_get_instance_private (self);
  ...
  GTK_WIDGET_CLASS (hdy_stack_parent_class)->unmap (widget);
}

static void
hdy_stack_switch_child (HdySwipeable *swipeable,
                        guint         index,
                        gint64        duration)
{
  HdyStack *self = HDY_STACK (swipeable);
  HdyStackPrivate *priv = hdy_stack_get_instance_private (self);
  HdyStackChildInfo *child_info;
...
static HdyStackChildInfo *
find_swipeable_child (HdyStack *self,
                      gint      direction)
{
  HdyStackPrivate *priv;
  GList *children;
  HdyStackChildInfo *child = NULL;
...
static double
get_current_progress (HdyStack *self)
{
  HdyStackPrivate *priv = hdy_stack_get_instance_private (self);
  gboolean new_first = FALSE;
  GList *children;
...
static void
hdy_stack_begin_swipe (HdySwipeable *swipeable,
                       gint          direction,
                       gboolean      direct)
{
  HdyStack *self = HDY_STACK (swipeable);
...
static void
hdy_stack_update_swipe (HdySwipeable *swipeable,
                        gdouble       value)
{
  HdyStack *self = HDY_STACK (swipeable);
  HdyStackPrivate *priv = hdy_stack_get_instance_private (self);
...
static void
hdy_stack_end_swipe (HdySwipeable *swipeable,
                     gint64        duration,
                     gdouble       to)
{
  HdyStack *self = HDY_STACK (swipeable);
...
static gboolean
captured_event_cb (HdyStack *self,
                   GdkEvent *event)
{
  HdyStackPrivate *priv = hdy_stack_get_instance_private (self);
...
static void
hdy_stack_class_init (HdyStackClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);
  GtkWidgetClass *widget_class = (GtkWidgetClass*) klass;
  GtkContainerClass *container_class = (GtkContainerClass*) klass;

  object_class->get_property = hdy_stack_get_property;
  object_class->set_property = hdy_stack_set_property;
  object_class->dispose = hdy_stack_dispose;
  object_class->finalize = hdy_stack_finalize;

  widget_class->realize = hdy_stack_realize;
  widget_class->unrealize = hdy_stack_unrealize;
  widget_class->map = hdy_stack_map;
  widget_class->unmap = hdy_stack_unmap;
  widget_class->get_preferred_width = hdy_stack_get_preferred_width;
  widget_class->get_preferred_height = hdy_stack_get_preferred_height;
  widget_class->get_preferred_width_for_height = hdy_stack_get_preferred_width_for_height;
  widget_class->get_preferred_height_for_width = hdy_stack_get_preferred_height_for_width;
  widget_class->size_allocate = hdy_stack_size_allocate;
  widget_class->draw = hdy_stack_draw;
  widget_class->direction_changed = hdy_stack_direction_changed;

  container_class->add = hdy_stack_add;
  container_class->remove = hdy_stack_remove;
  container_class->forall = hdy_stack_forall;
  container_class->set_child_property = hdy_stack_set_child_property;
  container_class->get_child_property = hdy_stack_get_child_property;
  ...
  props[PROP_TRANSITION_TYPE] =
    g_param_spec_enum ("transition-type",
                       ...
                       HDY_TYPE_STACK_TRANSITION_TYPE, HDY_STACK_TRANSITION_TYPE_NONE,
                       G_PARAM_READWRITE | G_PARAM_EXPLICIT_NOTIFY);
  ...
  gtk_widget_class_set_css_name (widget_class, "hdystack");
}

GtkWidget *
hdy_stack_new (void)
{
  return g_object_new (HDY_TYPE_STACK, NULL);
}

static void
hdy_stack_init (HdyStack *self)
{
  GtkWidget *widget = GTK_WIDGET (self);
  HdyStackPrivate *priv = hdy_stack_get_instance_private (self);
  ...
  priv->transition_type = HDY_STACK_TRANSITION_TYPE_NONE;
  priv->transition.type = HDY_STACK_TRANSITION_TYPE_NONE;
  ...
  priv->shadow_helper = hdy_shadow_helper_new (widget,
                                               "/sm/puri/handy/style/hdy-stack.css");
...
static void
hdy_stack_buildable_init (GtkBuildableIface *iface)
{
}

static void
hdy_stack_swipeable_init (HdySwipeableInterface *iface)
{
  iface->switch_child = hdy_stack_switch_child;
  iface->begin_swipe = hdy_stack_begin_swipe;
  iface->update_swipe = hdy_stack_update_swipe;
  iface->end_swipe = hdy_stack_end_swipe;
}
