...
/* The following constants are taken from WebKitGTK code.
 * See https://github.com/WebKit/webkit/blob/master/Source/WebKit/UIProcess/gtk/ViewGestureControllerGtk.cpp
 */
#define SHADOW_SIZE 81
#define SHADOW_OPACITY 0.06
#define DIMMING_OPACITY 0.12

#define SHADOW_GRADIENT_N_POINTS 16

static const double SHADOW_GRADIENT_OFFSETS[] = { 0, 0.03125, 0.0625, 0.0938, 0.125, 0.1875, 0.25, 0.375, 0.4375, 0.5, 0.5625, 0.625, 0.6875, 0.75, 0.875, 1.0 };
static const double SHADOW_GRADIENT_ALPHA[] = { 1, 0.99, 0.98, 0.95, 0.92, 0.82, 0.71, 0.46, 0.35, 0.25, 0.17, 0.11, 0.07, 0.04, 0.01, 0.0 };
                         ...
                         GtkPanDirection  direction)
{
  ...
  for (int i = 0; i < SHADOW_GRADIENT_N_POINTS; i++) {
    double offset, alpha;
                        ...
                        cairo_t   *cr)
{
  ...
  if (gtk_cairo_should_draw_window (cr, priv->bin_window)) {
    ...
    gtk_container_propagate_draw (GTK_CONTAINER (self),
                                  priv->visible_child->widget,
                                  cr);
    hdy_leaflet_draw_shadow (cr, clip_x, clip_y, clip_w, clip_h, distance, progress, priv->child_transition.active_direction);
    cairo_restore (cr);
  }
                       ...
                       cairo_t   *cr)
{
  ...
  if (priv->child_transition.last_visible_surface &&
    ...
    cairo_set_source_surface (cr, priv->child_transition.last_visible_surface, x, y);
    cairo_paint (cr);
    hdy_leaflet_draw_shadow (cr, clip_x, clip_y, clip_w, clip_h, distance, progress, direction);
    cairo_restore (cr);
  }
