 ...
 * another line that moves between them.
 *
 * # CSS nodes
 *
 * #HdyCarouselPageLines has a single CSS node with name carouselpagelines.
 *
 * Since: 1.0
 */

struct _HdyCarouselPageLines
{
...
G_DEFINE_TYPE_WITH_CODE (HdyCarouselPageLines, hdy_carousel_page_lines, GTK_TYPE_DRAWING_AREA,
...
animation_cb (GtkWidget     *widget,
              ...
              gpointer       user_data)
{
  HdyCarouselPageLines *self = HDY_CAROUSEL_PAGE_LINES (widget);
  gint64 frame_time;
...
static void
stop_animation (HdyCarouselPageLines *self)
{
...
static void
animate (HdyCarouselPageLines *self,
         gint64                duration)
{
  GdkFrameClock *frame_clock;
  gint64 frame_time;
...
static void
n_pages_changed_cb (HdyCarouselPageLines *self)
{
  animate (self, hdy_carousel_get_reveal_duration (self->carousel));
}

static void
hdy_carousel_page_lines_measure (GtkWidget      *widget,
                                GtkOrientation  orientation,
                                gint            for_size,
                                gint           *minimum,
                                gint           *natural,
                                ...
                                gint           *natural_baseline)
{
  ...
  if (orientation == self->orientation) {
    ...
    size = (LINE_LENGTH + LINE_SPACING) * n_pages - LINE_SPACING;
  } else {
    size = LINE_WIDTH;
  }
...
static void
hdy_carousel_page_lines_get_preferred_width (GtkWidget *widget,
                                            gint      *minimum_width,
                                            gint      *natural_width)
{
  hdy_carousel_page_lines_measure (widget, GTK_ORIENTATION_HORIZONTAL, -1,
                                  minimum_width, natural_width, NULL, NULL);
}

static void
hdy_carousel_page_lines_get_preferred_height (GtkWidget *widget,
                                             gint      *minimum_height,
                                             gint      *natural_height)
{
  hdy_carousel_page_lines_measure (widget, GTK_ORIENTATION_VERTICAL, -1,
                                  minimum_height, natural_height, NULL, NULL);
}

static gboolean
hdy_carousel_page_lines_draw (GtkWidget *widget,
                             cairo_t   *cr)
{
  HdyCarouselPageLines *self = HDY_CAROUSEL_PAGE_LINES (widget);
  gint i, n_points;
...
{
  HdyCarouselPageLines *self = HDY_CAROUSEL_PAGE_LINES (object);

  hdy_carousel_page_lines_set_carousel (self, NULL);

  G_OBJECT_CLASS (hdy_carousel_page_lines_parent_class)->dispose (object);
}

static void
hdy_carousel_page_lines_get_property (GObject    *object,
                                      guint       prop_id,
                                      GValue     *value,
                                      GParamSpec *pspec)
{
  ...
  switch (prop_id) {
  case PROP_CAROUSEL:
    g_value_set_object (value, hdy_carousel_page_lines_get_carousel (self));
    break;
...
static void
hdy_carousel_page_lines_set_property (GObject      *object,
                                      guint         prop_id,
                                      const GValue *value,
                                      GParamSpec   *pspec)
{
  ...
  switch (prop_id) {
  case PROP_CAROUSEL:
    hdy_carousel_page_lines_set_carousel (self, g_value_get_object (value));
    break;
...
static void
hdy_carousel_page_lines_class_init (HdyCarouselPageLinesClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);
  GtkWidgetClass *widget_class = GTK_WIDGET_CLASS (klass);

  object_class->dispose = hdy_carousel_dispose;
  object_class->get_property = hdy_carousel_page_lines_get_property;
  object_class->set_property = hdy_carousel_page_lines_set_property;

  widget_class->get_preferred_width = hdy_carousel_page_lines_get_preferred_width;
  widget_class->get_preferred_height = hdy_carousel_page_lines_get_preferred_height;
  widget_class->draw = hdy_carousel_page_lines_draw;

  /**
   * HdyCarouselPageLines:carousel:
   *
  ...
  gtk_widget_class_set_css_name (widget_class, "carouselpagelines");
}

static void
hdy_carousel_page_lines_init (HdyCarouselPageLines *self)
{
}

/**
 * hdy_carousel_page_lines_new:
 *
 * Create a new #HdyCarouselPageLines widget.
 *
 * Returns: (transfer full): The newly created #HdyCarouselPageLines widget
 *
 * Since: 1.0
 */
GtkWidget *
hdy_carousel_page_lines_new (void)
{
  return g_object_new (HDY_TYPE_CAROUSEL_PAGE_LINES, NULL);
}

/**
 * hdy_carousel_page_lines_get_carousel:
 * @self: a #HdyCarouselPageLines
 *
 * Get the #HdyCarousel the indicator uses.
 *
 * See: hdy_carousel_page_lines_set_carousel()
 *
...
HdyCarousel *
hdy_carousel_page_lines_get_carousel (HdyCarouselPageLines *self)
{
  g_return_val_if_fail (HDY_IS_CAROUSEL_PAGE_LINES (self), NULL);

  return self->carousel;
}

/**
 * hdy_carousel_page_lines_set_carousel:
 * @self: a #HdyCarouselPageLines
 ...
 */
void
hdy_carousel_page_lines_set_carousel (HdyCarouselPageLines *self,
                                      HdyCarousel          *carousel)
{
  g_return_if_fail (HDY_IS_CAROUSEL_PAGE_LINES (self));
