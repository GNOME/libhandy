 ...
 *
 * An adaptive view switcher, designed to switch between multiple main views in
 * a similar fashion than a #GtkStackSwitcher.
 *
 * Since: 0.0.10
 */
 ...
 * @HDY_VIEW_SWITCHER_POLICY_TYPE_AUTO: Automatically adapt to the best fitting mode
 * @HDY_VIEW_SWITCHER_POLICY_TYPE_NARROW: Force the narrow mode
 * @HDY_VIEW_SWITCHER_POLICY_TYPE_WIDE: Force the wide mode
 */
                                ...
                                GParamSpec *pspec)
{
  ...
  switch (prop_id) {
  case PROP_POLICY_TYPE:
    g_value_set_enum (value, hdy_view_switcher_get_policy_type (self));
    break;
  case PROP_ICON_SIZE:
                               ...
                               GParamSpec   *pspec)
{
  ...
  switch (prop_id) {
  case PROP_POLICY_TYPE:
    hdy_view_switcher_set_policy_type (self, g_value_get_enum (value));
    break;
  case PROP_ICON_SIZE:
                                       ...
                                       gint      *nat)
{
  ...
  switch (priv->policy_type) {
  case HDY_VIEW_SWITCHER_POLICY_TYPE_NARROW:
    *min = max_v_min * n_children;
    *nat = max_v_nat * n_children;
    break;
  case HDY_VIEW_SWITCHER_POLICY_TYPE_WIDE:
    *min = max_h_min * n_children;
    *nat = max_h_nat * n_children;
    break;
  case HDY_VIEW_SWITCHER_POLICY_TYPE_AUTO:
  default:
    *min = max_v_min * n_children;
    *nat = max_h_nat * n_children;
    break;
           ...
           gint             width)
{
  ...
  if (priv->policy_type == HDY_VIEW_SWITCHER_POLICY_TYPE_NARROW)
    return TRUE;
  else if (priv->policy_type == HDY_VIEW_SWITCHER_POLICY_TYPE_WIDE)
    return FALSE;
...
hdy_view_switcher_class_init (HdyViewSwitcherClass *klass)
{
  ...
  /**
   * HdyViewSwitcher:policy-type:
   *
   * The policy the view switcher should use to determine which mode to use.
   *
   * Since: 0.0.10
   */
  props[PROP_POLICY_TYPE] =
    g_param_spec_enum ("policy-type",
                       _("Policy type"),
                       _("The policy to determine the mode to use"),
                       HDY_TYPE_VIEW_SWITCHER_POLICY_TYPE, HDY_VIEW_SWITCHER_POLICY_TYPE_AUTO,
...
/**
 * hdy_view_switcher_get_policy_type:
 * @self: a #HdyViewSwitcher
 *
 * Gets the policy type of @self.
 *
 * Since: 0.0.10
 */
HdyViewSwitcherPolicyType
hdy_view_switcher_get_policy_type (HdyViewSwitcher *self)
{
  HdyViewSwitcherPrivate *priv;

  g_return_val_if_fail (HDY_IS_VIEW_SWITCHER (self), HDY_VIEW_SWITCHER_POLICY_TYPE_AUTO);

  priv = hdy_view_switcher_get_instance_private (self);

  return priv->policy_type;
}

/**
 * hdy_view_switcher_set_policy_type:
 * @self: a #HdyViewSwitcher
 * @policy_type: the new policy type
 *
 * Sets the policy type of @self.
 *
 * Since: 0.0.10
 */
void
hdy_view_switcher_set_policy_type (HdyViewSwitcher           *self,
                                   HdyViewSwitcherPolicyType  policy_type)
{
  HdyViewSwitcherPrivate *priv;
  ...
  if (priv->policy_type == policy_type)
    return;

  priv->policy_type = policy_type;

  g_object_notify_by_pspec (G_OBJECT (self), props[PROP_POLICY_TYPE]);
