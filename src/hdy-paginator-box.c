  ...
  for (children = self->children; children; children = children->next) {
    ...
    gtk_widget_set_child_visible (child,
                                  (self->orientation == GTK_ORIENTATION_HORIZONTAL &&
                                   x + width + self->spacing / 2.0 >= allocation->x &&
                                   x - self->spacing / 2.0 <= allocation->x + allocation->width) ||
                                  (self->orientation == GTK_ORIENTATION_VERTICAL &&
                                   y + height + self->spacing / 2.0 >= allocation->y &&
                                   y - self->spacing / 2.0 <= allocation->y + allocation->height));
...
static void
hdy_paginator_box_add (GtkContainer *container, GtkWidget *widget)
{
...
static void
hdy_paginator_box_remove (GtkContainer *container, GtkWidget *widget)
{
  HdyPaginatorBox *self = HDY_PAGINATOR_BOX (container);
                                ...
                                GParamSpec *pspec)
{
  ...
  switch (prop_id) {
  case PROP_N_PAGES:
    g_value_set_int (value, hdy_paginator_box_get_n_pages (self));
    break;
  ...
  case PROP_SPACING:
    g_value_set_int (value, hdy_paginator_box_get_spacing (self));
    break;
                                ...
                                GParamSpec   *pspec)
{
  ...
  case PROP_SPACING:
    hdy_paginator_box_set_spacing (self, g_value_get_int (value));
    break;
