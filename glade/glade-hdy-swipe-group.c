...
#define GLADE_TAG_SWIPEGROUP_WIDGETS "widgets"
#define GLADE_TAG_SWIPEGROUP_WIDGET  "widget"


static void
glade_hdy_swipe_group_read_widgets (GladeWidget *widget, GladeXmlNode *node)
{
  ...
  if ((widgets_node =
       glade_xml_search_child (node, GLADE_TAG_SWIPEGROUP_WIDGETS)) != NULL) {
    GladeXmlNode *n;
    ...
    for (n = glade_xml_node_get_children (widgets_node);
      ...
      if (!glade_xml_node_verify (n, GLADE_TAG_SWIPEGROUP_WIDGET))
        continue;
                                     ...
                                     GladeXmlNode    *node)
{
  ...
  widgets_node = glade_xml_node_new (context, GLADE_TAG_SWIPEGROUP_WIDGETS);
  ...
  if (glade_widget_property_get (widget, "widgets", &widgets)) {
    for (list = widgets; list; list = list->next) {
      ...
      widget_node =
        glade_xml_node_new (context, GLADE_TAG_SWIPEGROUP_WIDGET);
